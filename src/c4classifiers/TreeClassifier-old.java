...
public class TreeClassifier
{
	...
	private BinaryTree ID3(int examples[], int attributes[]) throws IOException //target_attribute is the same throughout
	{
		...
		if (line == null) //so I know it didn't break due to class change
			return (new BinaryTree(target_tracker[1])); // lonely root with label of class
		...
		if (temp != -1) // if there are no attributes
		{
			if (line == null) // if I was done reading the data. should I check row number == 67557 instead?
				return (new BinaryTree(majority_tracker > 0 ? "win" : "loss")); //single node with majority
		...
		return (new BinaryTree()); //dummy return statement, will remove later
	}
