...
//			previousF0X = F0X;
		  ...
		  // progress conditions
			...
			for (int j = 0; j < getMieq(); j++) {
				...
				//GradSum.assign(ALG.multOuter(g, g, null).assign(Functions.mult(-L.get(j) / fiX.get(j))), Functions.plus);
...
//			stepL = a1.assign(c1, Functions.minus);
			...
			while (cnt < 500) {
				cnt++;
				// X1 = X + s*stepX
				X1 = stepX.copy().assign(Functions.mult(s)).assign(X, Functions.plus);
